...
void dfs(int v, vector<vector<int>>& graph, vector<bool>& visited, vector<bool>& visit, vector<bool>& used, vector<int>& path, set<vector<int>>& cycles) {
    visited[v] = true;
    used[v] = true;
    path.push_back(v);
    ...
    saveCycleToFile(path);
    used[v] = false; // Позначаємо вершину як невідвідану для інших шляхів
}
...
vector<vector<int>> findAllCycles(vector<vector<int>>& graph) {
    ...
    for (int i = 1; i < n; ++i) {
        if (!visited[i]) {
            dfs(i, graph, visited, visit, used, path, cycles); // Запуск DFS для кожної вершин
        }
        visit[i] = true;
    }
